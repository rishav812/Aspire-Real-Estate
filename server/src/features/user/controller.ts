...
export const createListingApi = async (req: Request, res: Response) => {
  try {
    const response = await AuthServices.CreateListingService(req.body);
    res.status(200).json(response);
  } catch (error: any) {
    res.status(500).json({ message: error.message });
  }
...
export const getAllCountry = async (req: Request, res: Response) => {
  try {
    const response = await AuthServices.getCountriesService();
    res.status(200).json(response);
  } catch (error: any) {
    res.status(500).json({ message: error.message });
  }
